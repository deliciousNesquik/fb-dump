...
# ------------------------------------------------------------ shared pieces
def _comment_of(obj: Any, kind: str) -> str:
    return f"COMMENT ON {kind} {obj.get_quoted_name()} IS '{escape_single_quotes(obj.description)}'"
...
def _trigger(ctx: Context, tr: Any, path: str) -> Iterator[Artifact]:
    ...
    if external is None and not is_ddl:
        ...
        if relation is not None:
            header += f" FOR {relation.get_quoted_name()}"
