...
def test_table_grants_grouped_and_ordered():
    ...
    out = render_grants(privs, "relation", "T", owner="SYSDBA")
    assert out == [
        "GRANT SELECT ON T TO PUBLIC",
        "GRANT SELECT, INSERT ON T TO U1",
        "GRANT UPDATE (A, B) ON T TO ROLE R1",
    ]


def test_owner_filter_is_per_object_not_per_name():
    privs = [FPriv("SYSDBA", "S", "T")]
    assert render_grants(privs, "relation", "T", owner="OTHER") == ["GRANT SELECT ON T TO SYSDBA"]
...
def test_column_and_table_level_update_collapse_to_table_level():
    ...
    assert render_grants(privs, "relation", "T") == ["GRANT UPDATE ON T TO U"]
...
def test_grant_option_splits_statements():
    ...
    assert render_grants(privs, "relation", "T") == [
        ...
        "GRANT INSERT ON T TO U WITH GRANT OPTION",
    ]


def test_grantee_kinds_and_quoting():
    privs = [
    ...
    out = render_grants(privs, "relation", "T")
    assert out == [
...
def test_execute_and_usage_subjects():
    assert render_grants([FPriv("U", "X", "P", subject_type=5)], "procedure", "P") == ["GRANT EXECUTE ON PROCEDURE P TO U"]
    assert render_grants([FPriv("U", "X", "F", subject_type=15)], "function", "F") == ["GRANT EXECUTE ON FUNCTION F TO U"]
    assert render_grants([FPriv("U", "X", "K", subject_type=18)], "package", "K") == ["GRANT EXECUTE ON PACKAGE K TO U"]
    assert render_grants([FPriv("U", "G", "G1", subject_type=14)], "generator", "G1") == ["GRANT USAGE ON SEQUENCE G1 TO U"]
    assert render_grants([FPriv("U", "G", "E1", subject_type=7)], "exception", "E1") == ["GRANT USAGE ON EXCEPTION E1 TO U"]
...
def test_role_membership_default_and_admin():
    ...
    assert render_grants(privs, "role", "R", owner="SYSDBA") == [
        "GRANT R TO U1",
        "GRANT R TO U2 WITH ADMIN OPTION",
        "GRANT DEFAULT R TO U3",
        "GRANT R TO ROLE R2",
    ]


def test_grant_index_namespaces():
    idx = GrantIndex([
...
def test_database_level_grants():
    ...
    assert stmts == [
        "GRANT ALTER DATABASE TO U",
        "GRANT CREATE TABLE TO U",
        "GRANT ALTER ANY TABLE TO U",
        "GRANT DROP ANY PROCEDURE TO U",
        "GRANT CREATE CHARACTER SET TO U WITH GRANT OPTION",
    ]
    assert skipped == 1
